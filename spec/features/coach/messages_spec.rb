...
feature "coach messages" do
  ...
  it "displays study id but not email addresses of patients" do
    ...
    with_scope "#inbox" do
      expect(page).to have_content(users(:user1).received_messages.last.created_at.to_formatted_s(:short))
    end
  end
  ...
  it "allows a coach to reply to a message" do
    ...
    with_scope "#inbox" do
      expect(page).to have_content(users(:user1).messages.last.created_at.to_formatted_s(:short))
    end
