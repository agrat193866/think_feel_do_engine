...
feature "Activities", type: :feature do
  ...
  describe "When in Do #1 Awareness" do
    describe "ContentProvider::ActivityIndexProvider" do
      ...
      before :each do
        ...
        Activity.create(
          ...
          participant: participant1,
          start_time: DateTime.current.advance(hours: -106),
          end_time: DateTime.current.advance(hours: -105))
        Activity.create(
      ...
      scenario "Participant views activities index and sees recent activities" do
        sign_in_participant participant1
        visit "/navigator/modules/#{ bit_core_content_modules(:do_awareness).id }" \
              "/providers/" \
              "#{ bit_core_content_providers(:activity_index).id }/1"

        expect(page).not_to have_text("Arguing with my boss")
